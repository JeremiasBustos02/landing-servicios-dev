...
import { NAV_LINKS, EASE } from '@/data'
import BrandLogo from '@/components/ui/BrandLogo'
import PrimaryButton from '@/components/ui/PrimaryButton'
import { Menu, X } from 'lucide-react'
import type { NavLink, SwitchOption } from '@/types'
...
function DesktopNavLink({
  ...
  link: NavLink;
  onScroll: (e: React.MouseEvent<HTMLAnchorElement>, target: string, tab?: SwitchOption) => void;
}) {
  if (link.groups) {
    return (
                ...
                <div className="flex flex-col gap-1">
                  {group.items.map(item => (
                    <a
                      ...
                      onClick={(e) => onScroll(e, item.href, item.tab)}
                    >
                      {item.label}
                    </a>
                  ))}
        ...
        link.groups ? (
          <div key={link.label} className="flex flex-col">
            <button
              ...
              onClick={() => onToggleGroup(link.label)}
    ...
    >
      <div className="navbar">
        ...
        <div className="navbar-links">
          {NAV_LINKS.map(link => (
            <DesktopNavLink key={link.href || link.label} link={link} onScroll={handleScroll} />
          ))}
        </div>
      ...
      >
        <MobileMenu
          ...
          onToggleGroup={(label) => setExpandedGroup(expandedGroup === label ? null : label)}
